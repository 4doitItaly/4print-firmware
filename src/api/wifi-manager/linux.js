...
(function() {
  ...
  module.exports = {
    autoFindInterface: function() {
      ...
      findInterfaceCom = "export LC_ALL=C && nmcli -m multiline device status | grep wlan && unset LC_ALL";
    ...
    getIfaceState: function() {
      ...
      interfaceState = {};
      powerData = this.execSync("export LC_ALL=C && nmcli networking && unset LC_ALL");
      interfaceState.power = powerStateMap[powerData.trim()];
      if (interfaceState.power) {
        foundInterface = false;
        connectionData = this.execSync("export LC_ALL=C && nmcli -m multiline device status && unset LC_ALL");
        connectionName = null;
        ...
        if (connectionName) {
          try {
            ssidData = this.execSync("export LC_ALL=C && nmcli -m multiline connection show \"" + connectionName + "\" | grep 802-11-wireless.ssid && unset LC_ALL");
    ...
    connectToAP: function(_ap) {
      ...
      COMMANDS = {
        "delete": "export LC_ALL=C && nmcli connection delete \"" + _ap.ssid + "\" && unset LC_ALL",
        connect: "export LC_ALL=C && nmcli device wifi connect \"" + _ap.ssid + "\" "
      };
      if (_ap.password.length) {
        COMMANDS.connect += " password \"" + _ap.password + "\" && unset LC_ALL";
      } else {
        COMMANDS.connect += " && unset LC_ALL";
      }
      try {
        stdout = this.execSync("export LC_ALL=C && nmcli connection show \"" + _ap.ssid + "\" && unset LC_ALL");
        if (stdout.length) {
          ssidExist = true;
        }
      } catch (error1) {
    ...
    resetWiFi: function() {
      ...
      COMMANDS = {
        disableNetworking: "export LC_ALL=C && nmcli networking off && unset LC_ALL",
        enableNetworking: "export LC_ALL=C && nmcli networking on && unset LC_ALL"
      };
